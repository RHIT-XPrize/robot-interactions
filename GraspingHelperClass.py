...
def load_objects(scene, planning_frame):
    ...
    if debugMode == 1:
        ...
        z = 34
        box_pose.header.frame_id = planning_frame  # must put it in the frame
        box_pose.pose.position.x = in_to_m(39) + in_to_m(x / 2) 
        box_pose.pose.position.y = in_to_m(-38) + in_to_m(y / 2) 
        box_pose.pose.position.z = in_to_m(-36) + in_to_m(z / 2)
        box_name = "desktop table"
        scene.add_box(box_name, box_pose, (in_to_m(x), in_to_m(y), in_to_m(z)))
        ...
        z = 34
        box_pose.header.frame_id = planning_frame  # must put it in the frame
        box_pose.pose.position.x = in_to_m(9) + in_to_m(x / 2) 
        box_pose.pose.position.y = in_to_m(-38) + in_to_m(y / 2)
        box_pose.pose.position.z = in_to_m(-36) + in_to_m(z / 2)
        box_name = "operating table"
        scene.add_box(box_name, box_pose, (in_to_m(x), in_to_m(y), in_to_m(z)))
        ...
        box_pose = PoseStamped()
        box_pose.header.frame_id = planning_frame  # must put it in the frame
        box_pose.pose.position.x = in_to_m(-150) + in_to_m(x / 2) 
        box_pose.pose.position.y = in_to_m(-38) + in_to_m(y / 2) 
        ...
        box_pose = PoseStamped() 
        box_pose.header.frame_id = planning_frame  # must put it in the frame 
        box_pose.pose.position.x = in_to_m(-26) + in_to_m(x / 2) 
        ...
        box_pose.header.frame_id = planning_frame  # must put it in the frame
        box_pose.pose.position.x = in_to_m(-26) + in_to_m(x / 2)
        box_pose.pose.position.y = in_to_m(-28) + in_to_m(-y / 2) 
        box_pose.pose.position.z = in_to_m(-150) + in_to_m(z / 2)
        box_name = "pillar"
        scene.add_box(box_name, box_pose, (in_to_m(x), in_to_m(y), in_to_m(z)))
...
def remove_objects(scene):
    ...
    if debugMode == 1:
        ...
        scene.remove_world_object("desktop table")
        # rospy.sleep(1)
        scene.remove_world_object("operating table")
        # rospy.sleep(1)
        scene.remove_world_object("wall side")
        # rospy.sleep(1)
        scene.remove_world_object("wall back")
        # rospy.sleep(1)
